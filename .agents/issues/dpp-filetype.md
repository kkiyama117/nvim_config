...
| C2 | Folding nearly free: `foldmethod=marker` with default marker `{{{,}}}` folds exactly the hook blocks (incl. nested). | DONE: set in `after/ftplugin/dpp.lua` (`foldmethod=marker`, explicit `foldmarker={{{,}}}`, `foldlevelstart=99`). Verified: zM closes 1-3 and 4-8, zR opens. |
