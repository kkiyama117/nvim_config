...
vim.keymap.set(
  ...
  '>',
  '>gv',
  { desc = 'Insert indents with viusal mode' }
)
vim.keymap.set(
  { 'x' },
  '<',
  '<gv',
  { desc = 'Remove indents with visual mode' }
)
-- }}}
...
vim.keymap.set(
  ...
  'su',
  ':s//g<Left><Left>',
  { desc = 'substitute with last search pattern' }
)
...
vim.keymap.set('n', '[TOGGLE]h', function()
  ...
  vim.notify(
    string.format('messagesopt=%s', vim.opt.messagesopt:get()),
    vim.log.levels.INFO
  )
  vim.notify(
    string.format('more=%s', tostring(vim.opt.more:get())),
    vim.log.levels.INFO
  )
...
-- restart
vim.keymap.set('n', 'qr', '<Cmd>:restart +xall<CR>', { desc = 'restart' })
